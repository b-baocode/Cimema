...
namespace MV.ApplicationLayer.Services
{
    public class RegisterService : IRegisterService
    {
        ...
        public async Task<string> RegisterUser(RegisterRequest registerRequest)
        {
            ...
            if (registerRequest.Password.Length < 6 || registerRequest.Password.Length > 100)
            {
                return "Password must be between 6-100 characters";
            }

            if (!Regex.IsMatch(registerRequest.Password, @"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$"))
            {
                return "Password must contain at least 1 uppercase letter, 1 lowercase letter, 1 number and 1 special character";
            }

            // Validate identity number
            if (string.IsNullOrEmpty(registerRequest.Identitynumber))
            {
                return "Identity number is required";
            }

            if (!Regex.IsMatch(registerRequest.Identitynumber, @"^\d{12}$"))
            {
            ...
            if (!Regex.IsMatch(registerRequest.Email, @"^[a-zA-Z0-9._%+-]+@gmail\.com$"))
            {
            ...
            if (!Regex.IsMatch(registerRequest.Phone, @"^0\d{9}$"))
            {
            ...
            if (registerRequest.Birthdate > DateOnly.FromDateTime(DateTime.Now))
            {
